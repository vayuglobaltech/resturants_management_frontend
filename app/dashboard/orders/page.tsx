...
const ORDER_STATUSES = ["PENDING", "CONFIRMED", "QUEUED", "PREPARING", "READY", "DELIVERED", "COMPLETED","PAID", "CANCELLED"];

const statusColors: Record<string, string> = {
  PENDING: "bg-amber-500/10 text-amber-400 border-amber-500/20",
  CONFIRMED: "bg-blue-500/10 text-blue-400 border-blue-500/20",
  QUEUED: "bg-indigo-500/10 text-indigo-400 border-indigo-500/20",     // added
  ...
  PAID: "bg-green-500/10 text-green-400 border-green-500/20",          // added
...
export default function OrdersPage() {
  ...
  // Filter orders by table if tableParam exists
  const filteredOrders = tableParam
    ? allOrders.filter(
        (o) => o.table === parseInt(tableParam) || o.table?.id === parseInt(tableParam)
      )
    : allOrders;
  ...
  const otherStatuses = Array.from(
    new Set(filteredOrders.map((o) => o.status).filter((s) => !ORDER_STATUSES.includes(s)))
  );
  ...
  // Determine if we have any orders to show
    ...
    <div className="flex flex-col h-full space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3 flex-shrink-0">
        <h1 className="text-2xl font-bold text-white flex items-center gap-2">
          ...
          {tableParam ? `Orders for Table ${tableParam}` : "All Orders"}
        </h1>
      ...
      {!hasOrders ? (
        <div className="text-center py-16 border border-dashed border-white/10 rounded-2xl flex-shrink-0">
          <p className="text-slate-400">
            {tableParam
              ? `No orders found for Table ${tableParam}.`
        ...
        <div className="flex flex-1 overflow-x-auto pb-4 gap-6 snap-x">
          {ordersByStatus
            .filter(
              (col) =>
                col.items.length > 0 || ORDER_STATUSES.slice(0, 4).includes(col.status)
            )
            .map((column) => (
                    ...
                    <span
                      className={cn(
                        ...
                        statusColors[column.status]
                          ? statusColors[column.status].split(" ")[0].replace("/10", "")
                          : "bg-slate-500"
                      )}
                    />
                          ...
                          <div className="flex justify-between items-start mb-2">
                            <div>
                              ...
                              <p className="text-xs text-slate-400 mt-0.5">
                                Table{" "}
                                {order.table_number_display || order.table_number || "N/A"}
                              </p>
                            </div>
                  ...
                  {column.items.length === 0 && (
                    <div className="border border-dashed border-white/5 rounded-xl p-4 text-center">
                      <p className="text-xs text-slate-500">
                        No {column.status.toLowerCase()} orders
                      </p>
                    </div>
                  )}
                </div>
