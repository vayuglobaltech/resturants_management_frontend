...
function formatSmartTimestamp(dateString: string): string {
  ...
  if (isToday) {
    // Show only time in local format
...
export default function TableDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // // ─── Determine timestamp label based on status ──────────────────────
  // let statusLabel = "";
  // let timestamp = null;

  // // Always try to show a timestamp – use the most appropriate field
  ...
  //   // Fallback: show last updated time
  //   statusLabel = "Last Updated";
  //   timestamp = table.updated_at;
  // } else {
  //   // No timestamp available – hide the row
  //   statusLabel = "";
  //   timestamp = null;
  // }
